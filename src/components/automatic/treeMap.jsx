...
function Treemap_({ apiKey }) {
  ...
  const surfaceCountryData = intValuesCountryData
    ...
    .filter(
      (country) =>
        country.Series === "Surface area (thousand km2)" &&
        country.Year === 2020
    )
    .map((elem, i) => {
  ...
  const autovizData = Object.fromEntries(
    surfaceCountryData
      ...
      .map((country, i) => [country["Country"], country.SurfaceValue])
  );
  ...
  const root = hierarchy(dataStratified).sort(
  ...
  return (
    <div style={{ textAlign: "left" }}>
      ...
      <p style={{ marginBottom: 4, marginTop: 4, fontSize: 12 }}>
