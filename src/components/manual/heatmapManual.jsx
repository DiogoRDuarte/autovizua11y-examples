...
function HeatmapManual({ longDesc, shortDesc }) {
  ...
  function generator(year) {
    return genBins(
      ...
      /* height = */ 10,
      /** binFunc */ (idx) => idx,
      /** countFunc */ (i, number) => filteredCountryData[i][year]
    );
  }
  let binData = [];
  binData.push(
    generator("2010")[0],
    generator("Value2015")[0],
    generator("Value")[0]
  );
  binData.map((bin, i) => {
    bin.bin = i;
  ...
  const yaxisScale = scaleBand({
    range: [0, yMax],
    domain: filteredCountryData.map(
      (country) => country["Population, density and surface area"]
    ),
  });
    ...
    <div style={{ textAlign: "left" }}>
      ...
      >
        <svg width={chartDimensions.width} height={chartDimensions.height}>
          <Group
            top={chartDimensions.marginTop + 20}
            left={chartDimensions.marginLeft + 70}
          >
            <HeatmapRect
              data={binData}
              xScale={(d) => xScale(d) ?? 0}
