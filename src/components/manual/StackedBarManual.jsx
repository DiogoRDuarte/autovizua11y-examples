...
function StackedBarManual({ longDesc, shortDesc }) {
  ...
  const sexRatioData = countryData.filter(
    (country) =>
      country.Series === "Sex ratio (males per 100 females)" &&
      country.Year === 2022
  );
  ...
  mergedAux = mergedAux
    ...
    .flat()
    .map((item, i) => {
      const RatioMale = Math.round(
        (item.SexRatio * 100) / (item.SexRatio + 100)
      );
  ...
  return (
    <div style={{ textAlign: "left" }}>
      ...
      <AutoVizuA11y
        ...
        descriptor=""
        insights=""
        title="The ratio of male to female varies significantly in some countriess"
        context={
      ...
      >
        <svg width={chartDimensions.width} height={chartDimensions.height}>
          <Group
            left={chartDimensions.marginLeft + 65}
          ...
          >
            ...
            {mergedData
              ...
              .map((rect, i) => {
                const sexRatioMale =
                  (rect.SexRatio * 100) / (rect.SexRatio + 100);
...
export default StackedBarManual;
